...
export default function ReportsScreen() {
  ...
  const exportAs = async (format: 'PDF' | 'Excel') => {
    ...
    // On native this would use expo-print / expo-sharing — show confirmation for now
    Alert.alert(
      `Export ${format}`,
      `${reportRows.length} reading(s) from ${formatDate(startDate)} to ${formatDate(endDate)} ready for export.\n\n(File export requires expo-print & expo-sharing — integrate when building for production.)`,
      [{ text: 'OK' }]
    );
  };
